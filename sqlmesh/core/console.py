...
class TerminalConsole(Console):
    ...
    def log_warning(self, message: str) -> None:
        ...
        if not self.ignore_warnings:
            ...
            message_lines = [
                textwrap.fill(line, width=LINE_WRAP_WIDTH, replace_whitespace=False)
                for line in message_lstrip.split("\n")
            ]
            message_wrapped = "[yellow]\\[WARNING] " + "\n".join(message_lines) + "[/yellow]"
            self._print(leading_whitespace + message_wrapped)
